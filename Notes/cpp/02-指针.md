...
本篇以指针为主线，从函数签名建立这套阅读方法；引用作为对照简要出现，完整规则见第 03 篇。
...
`reinterpret_cast` 更接近底层表示层面的转换，偶尔出现在函数指针注册、内存映射等 ABI 边界。它不会验证目标对象的类型或生命周期。
...
指针转换与 cv 限定见 C++ 草案的 [conv.ptr](https://eel.is/c++draft/conv.ptr) 和 [dcl.type.cv](https://eel.is/c++draft/dcl.type.cv)。引用规则见第 03 篇，对象有效期见第 05 篇。
